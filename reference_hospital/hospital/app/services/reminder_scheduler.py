...
def _check_no_show_appointments():
    """يشيك على المواعيد التي مضى عليها أكثر من 60 دقيقة دون إنهاء أو حضور ويحولها لـ no_show."""
    now = datetime.now()
    ...
    try:
        appointments = db.query(Appointment).filter(
            Appointment.status.in_([AppointmentStatus.pending, AppointmentStatus.confirmed]),
            Appointment.appointment_date <= threshold
        ).all()
        ...
        if appointments:
            db.commit()
            print(f"[Reminder] Updated {len(appointments)} appointments to no_show status")
    except Exception as e:
        print(f"[Reminder] Error checking no-show appointments: {e}")
        db.rollback()
    finally:
        db.close()
