        ...
        'data': [
            {
                ...
                'bio': d.bio or '',
                'consultation_price': float(d.consultation_price) if d.consultation_price is not None else None
            }
            for d in doctors
        ]
    }
...
async def book_appointment(
    ...
    try:
        # Get patient profile if user is patient, else check payload for patient
        patient = None
        if current_user.role == UserRole.patient:
            patient = db.query(Patient).filter(Patient.user_id == current_user.id).first()
        elif current_user.role in [UserRole.receptionist, UserRole.clinic_owner]:
            # Try to get or create patient by name/phone or use existing first patient
            patient = db.query(Patient).first()

        if not patient:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={'success': False, 'message': 'حساب المريض غير متاح'}
            )
        ...
        # Create Invoice immediately upon booking with pending status
        if doctor.clinic_id:
            from app.models.billing import Invoice, PaymentStatus
            price = doctor.consultation_price or Decimal('200.00')
            inv = Invoice(
...
async def mark_patient_done(
    ...
    try:
        ...
        if inv and inv.payment_status != PaymentStatus.paid:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={'success': False, 'message': 'عذراً، لم يتم سداد فاتورة الكشف لدى موظف الاستقبال بعد'}
            )
        ...
        # Create invoice automatically for completed appointment
        from app.services.billing_service import BillingService
        BillingService.create_invoice_for_appointment(db, appt)
