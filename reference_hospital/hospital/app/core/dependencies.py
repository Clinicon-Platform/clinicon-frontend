...
def require_receptionist(current_user: User = Depends(get_current_user)) -> User:
    """يتأكد أن اليوزر موظف استقبال"""
    if _normalize_role(current_user.role) != UserRole.receptionist:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="هذا الإجراء مخصص لموظفي الاستقبال فقط")
    return current_user


def require_receptionist_or_owner(current_user: User = Depends(get_current_user)) -> User:
    """يتأكد أن اليوزر صاحب عيادة أو موظف استقبال"""
    r = _normalize_role(current_user.role)
    if r not in [UserRole.clinic_owner, UserRole.receptionist]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="هذا الإجراء مخصص لأصحاب العيادات وموظفي الاستقبال فقط")
    return current_user
